...
| `filter` | object | No | Filtro a aplicar a la búsqueda |
| `includeValues` | boolean | No | Incluir valores de vector en la respuesta \(predeterminado: true\) |
| `includeMetadata` | boolean | No | Incluir metadatos en la respuesta \(predeterminado: true\) |
| `apiKey` | string | Sí | Clave API de Pinecone |
