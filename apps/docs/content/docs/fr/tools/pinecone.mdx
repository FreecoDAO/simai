...
| `filter` | objet | Non | Filtre à appliquer à la recherche |
| `includeValues` | booléen | Non | Inclure les valeurs des vecteurs dans la réponse \(par défaut : true\) |
| `includeMetadata` | booléen | Non | Inclure les métadonnées dans la réponse \(par défaut : true\) |
| `apiKey` | chaîne | Oui | Clé API Pinecone |
